...
window.TransferUI = class TransferUI {
    ...
    bindEventListeners() {
        ...
        // Replace startTransferBtn's functionality with a validation-only version
        if (this.elements.startTransferBtn) {
            this.elements.startTransferBtn.addEventListener('click', () => {
                this.addLogEntry("Validating project...");
                // Trigger the event for the core to handle validation and database saving
        ...
        if (this.elements.resetStateBtn) {
            this.elements.resetStateBtn.addEventListener('click', () => {
                if (confirm('Are you sure you want to reset the project? All data will be cleared.')) {
                    this.addLogEntry("Resetting project state...");
                    this.eventManager.publish('reset-state-clicked');
                }
            });
        }
        
        // Replace cancelTransferBtn's functionality (we can hide or disable it since we removed transfer)
        if (this.elements.cancelTransferBtn) {
            this.elements.cancelTransferBtn.style.display = 'none'; // Hide the cancel button
            this.elements.cancelTransferBtn.addEventListener('click', () => {
                this.addLogEntry("Cancel functionality has been removed");
                this.showNotification('Cancel functionality has been removed', 'info');
                this.eventManager.publish('cancel-transfer-clicked');
            });
        }
        
        // Workflow navigation - Continue to next step
        if (this.elements.toStep2Btn) {
            this.elements.toStep2Btn.addEventListener('click', () => {
    ...
    handleNavigateToNextStep() {
        // Verify that we have a valid project before allowing navigation
        if (this.elements.toStep2Btn.disabled) {
            this.showNotification('Please validate the project first before continuing', 'warning');
            return;
        }
        
        // Publish an event that the core can listen to for navigation
        this.eventManager.publish('navigate-to-next-step');
        
        // Note: We don't need to handle the actual navigation here
        // The navigation will be handled by the core through the event system
        // This prevents duplicate navigation attempts
        this.addLogEntry("Navigation event published, waiting for core to handle navigation...");
        
        // The following code is now commented out since it's handled by the core's event handler
        // and this was causing the navigation to happen without parameters
        /*
        // If using multiple steps in a single page setup
        const currentStep = document.querySelector('.workflow-step.active');
        const nextStep = document.getElementById('step-2');
        
        if (currentStep && nextStep) {
            currentStep.classList.remove('active');
            nextStep.classList.add('active');
            this.addLogEntry("Navigated to Document Analysis step");
        } else {
            // If using different pages, redirect to the next step
            this.addLogEntry("Navigating to Document Analysis page...");
            window.location.href = '/register/document/';
        }
        */
    }
}
