...
DJANGO_CHECK_URL = f"http://localhost:{PORT}"  # Use localhost for connectivity checks
...
def wait_for_server(url, timeout=60):
    ...
    print_status(f"Waiting for server at {url} to respond...", Colors.YELLOW)
    start = time.time()
    while time.time() - start < timeout:
        try:
            urllib.request.urlopen(url, timeout=1)
            print_status(f"Server at {url} is responding!", Colors.GREEN)
            return True
        except Exception as e:
            elapsed = int(time.time() - start)
            if elapsed % 5 == 0:  # Print status every 5 seconds
                print_status(f"Still waiting for server... ({elapsed}s elapsed)", Colors.YELLOW)
            time.sleep(0.5)
    print_status(f"Timed out waiting for server at {url}", Colors.RED)
    return False
...
def run_django():
    ...
    if not wait_for_server(DJANGO_CHECK_URL):
...
def open_chrome_normal(url):
    """Open the default browser in normal mode pointing to `url`."""
    print_status(f"Opening default browser at {url}", Colors.MAGENTA)
    try:
        # Try to open with the default browser
        result = webbrowser.open(url)
        if result:
            print_status("Browser opened successfully", Colors.GREEN)
            return True
        else:
            print_status("webbrowser.open() returned False - browser may not have opened", Colors.YELLOW)
            # Try alternative method
            print_status("Trying alternative browser opening method...", Colors.YELLOW)
            webbrowser.open_new_tab(url)
            return True
    except Exception as e:
        print_status(f"Failed to open browser: {e}", Colors.RED)
        print_status("You may need to manually open your browser", Colors.YELLOW)
        return False

if __name__ == "__main__":
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Launch Micro Film application')
    parser.add_argument('-k', '--kiosk', action='store_true', help='Open browser in kiosk mode')
    parser.add_argument('-c', '--caddy', action='store_true', help='Use Caddy reverse proxy')
    ...
    if args.caddy:
        ...
        target_url = DJANGO_CHECK_URL  # Use localhost URL for browser

    # Clean‐up handler for both processes
    def handle_exit(sig, frame):
        print_header("SHUTTING DOWN", Colors.RED)
        processes = [("Django", django_proc)]
        if caddy_proc:
            processes.append(("Caddy", caddy_proc))
    ...
    # Give Django a moment to fully start before opening browser
    print_status("Waiting for Django to be fully ready...", Colors.YELLOW)
    time.sleep(2)

    # Open browser based on flags
    print_status(f"Attempting to open browser with URL: {target_url}", Colors.YELLOW)
    if args.kiosk:
        success = open_chrome_kiosk(target_url)
    else:
        success = open_chrome_normal(target_url)
        
    if not success:
        print_status("Failed to open browser. You can manually navigate to:", Colors.RED)
        print_status(f"  {target_url}", Colors.CYAN)
        print_status("Continuing without browser...", Colors.YELLOW)
