...
Namespace fileconverter
	...
	Friend NotInheritable Module modExpose
		' Token: 0x06000E1E RID: 3614 RVA: 0x0006FF3C File Offset: 0x0006E13C
		...
		' Token: 0x06000E1F RID: 3615 RVA: 0x000794C0 File Offset: 0x000776C0
		Public Function SetContinuationInfoFileForRefilm(ByRef roll As String, ByRef Entry As String) As Object
			Dim num As Integer
			Dim num4 As Integer
			Dim obj As Object
			Try
				...
				IL_27:
				num2 = 3
				Dim lpFileName As String = MyProject.Application.Info.DirectoryPath + "\ROLLDESCS\" + roll + ".desc"
				IL_4B:
				num2 = 4
				Dim flag As Boolean = -(modDeclares.WritePrivateProfileString("INFO", "CONTINUATION", Entry, lpFileName) > False)
				IL_65:
				GoTo IL_C8
				IL_67:
		...
		' Token: 0x06000E20 RID: 3616 RVA: 0x000795B0 File Offset: 0x000777B0
		<MethodImpl(MethodImplOptions.NoInlining Or MethodImplOptions.NoOptimization)>
		Public Function GetNumberOfStartSymbols(ByRef p As String, ByRef doc As Boolean) As Integer
			Dim num As Integer = CType(New FileSystemObjectClass(), IFileSystem3).GetFolder(p).Files.Count
			If Operators.CompareString(FileSystem.Dir(p + "\thumbs.db", Microsoft.VisualBasic.FileAttribute.Normal), "", False) <> 0 Then
				num -= 1
			End If
			Return num
		End Function

		' Token: 0x06000E21 RID: 3617 RVA: 0x000795FC File Offset: 0x000777FC
		Public Function EOFReached(ByRef restlen As Double, ByRef restbelege As Integer, ByRef curindex As Integer) As Boolean
			Dim result As Boolean = False
			If modDeclares.SystemData.FesteBelegzahlProFilm Then
				If restbelege = 0 Then
					result = True
				End If
			ElseIf modDeclares.SystemData.EnableRollFrameExt And curindex > 0 Then
				' The following expression was wrapped in a checked-expression
				If Operators.CompareString(modDeclares.Images(curindex).DokumentName, modDeclares.EndFolder, False) = 0 And modDeclares.Images(curindex + 1).Level = 2S Then
					result = True
				End If
			ElseIf restlen < CDbl(modDeclares.SystemData.nachspann) / 100.0 Then
				result = True
			End If
			Return result
		End Function

		' Token: 0x06000E22 RID: 3618 RVA: 0x0007968C File Offset: 0x0007788C
		<MethodImpl(MethodImplOptions.NoInlining Or MethodImplOptions.NoOptimization)>
		Public Function AddTrailer(ByRef roll As String) As Object
			Dim num4 As Integer
			Dim num6 As Integer
			Dim obj As Object
			Try
				...
				If Operators.CompareString(FileSystem.Dir(modDeclares.SystemData.LogfileName + "\" + roll + ".Ini", Microsoft.VisualBasic.FileAttribute.Normal), "", False) = 0 Then
					GoTo IL_43B
				End If
				IL_33:
				num = 3
				...
				IL_55:
				num = 5
				Dim num2 As Integer = CInt((modMain.TrailerCount - 1S))
				For i As Integer = 0 To num2
					IL_65:
					num = 6
					If modDeclares.SystemData.TrailerSel(i) Then
						IL_76:
						num = 7
						flag = True
					End If
				...
				IL_93:
				num = 10
				Using streamWriter As StreamWriter = New StreamWriter(modDeclares.SystemData.LogfileName + "\" + roll + ".txt", True, Encoding.Unicode)
					Dim num3 As Integer = CInt((modMain.TrailerCount - 1S))
					For i As Integer = 0 To num3
						If modDeclares.SystemData.TrailerSel(i) Then
							Dim text2 As String = modMain.GiveIni(text, "frmFilmPreview", "lstTrailer.0")
							If Operators.CompareString(text2, "", False) = 0 Then
								text2 = "Erster Dateiname auf Film"
							End If
							If Operators.CompareString(modDeclares.SystemData.Trailers(i), text2, False) = 0 Then
								Dim textWriter As TextWriter = streamWriter
								Dim str As String = text2
								Dim str2 As String = "="
								Dim text3 As String = modDeclares.SystemData.LogfileName + "\" + roll + ".Ini"
								textWriter.WriteLine(str + str2 + modMain.GiveIniW(text3, "INFO", "FIRSTFILE"))
							End If
							text2 = modMain.GiveIni(text, "frmFilmPreview", "lstTrailer.6")
							If Operators.CompareString(modDeclares.SystemData.Trailers(i), text2, False) = 0 Then
								streamWriter.WriteLine(text2 + "=" + Conversions.ToString(DateAndTime.Today))
							End If
							text2 = modMain.GiveIni(text, "frmFilmPreview", "lstTrailer.7")
							If Operators.CompareString(modDeclares.SystemData.Trailers(i), text2, False) = 0 Then
								streamWriter.WriteLine(text2 + "=" + Conversions.ToString(DateAndTime.Now))
							End If
							text2 = modMain.GiveIni(text, "frmFilmPreview", "lstTrailer.1")
							If Operators.CompareString(text2, "", False) = 0 Then
								text2 = "Letzter Dateiname auf Film"
							End If
							If Operators.CompareString(modDeclares.SystemData.Trailers(i), text2, False) = 0 Then
								Dim textWriter2 As TextWriter = streamWriter
								Dim str3 As String = text2
								Dim str4 As String = "="
								Dim text3 As String = modDeclares.SystemData.LogfileName + "\" + roll + ".Ini"
								textWriter2.WriteLine(str3 + str4 + modMain.GiveIniW(text3, "INFO", "LASTFILE"))
							End If
							text2 = modMain.GiveIni(text, "frmFilmPreview", "lstTrailer.3")
							If Operators.CompareString(text2, "", False) = 0 Then
								text2 = "Anzahl verfilmte Images"
							End If
							If Operators.CompareString(modDeclares.SystemData.Trailers(i), text2, False) = 0 Then
								Dim textWriter3 As TextWriter = streamWriter
								Dim str5 As String = text2
								Dim str6 As String = "="
								Dim text3 As String = modDeclares.SystemData.LogfileName + "\" + roll + ".Ini"
								textWriter3.WriteLine(str5 + str6 + modMain.GiveIniW(text3, "INFO", "IMAGECOUNT"))
							End If
							text2 = modMain.GiveIni(text, "frmFilmPreview", "lstTrailer.2")
							If Operators.CompareString(text2, "", False) = 0 Then
								text2 = "Anzahl bel. Frames"
							End If
							If Operators.CompareString(modDeclares.SystemData.Trailers(i), text2, False) = 0 Then
								Dim textWriter4 As TextWriter = streamWriter
								Dim str7 As String = text2
								Dim str8 As String = "="
								Dim text3 As String = modDeclares.SystemData.LogfileName + "\" + roll + ".Ini"
								textWriter4.WriteLine(str7 + str8 + modMain.GiveIniW(text3, "INFO", "FRAMECOUNT"))
							End If
						End If
					Next
					streamWriter.Close()
				...
				IL_39E:
				num = 12
				If Not File.Exists(modDeclares.SystemData.LogfileName + "\" + roll + ".Ini") Then
					GoTo IL_3E6
				End If
				IL_3C3:
				num = 13
				FileSystem.Kill(modDeclares.SystemData.LogfileName + "\" + roll + ".Ini")
				IL_3E6:
				ProjectData.ClearProjectError()
				num4 = 0
				IL_3ED:
				num = 15
				If Not modDeclares.SystemData.EXCELPROTOCOL Then
					GoTo IL_43B
				End If
				IL_3FC:
				num = 16
				modMain.ConvertLogToExcel(modDeclares.SystemData.LogfileName + "\" + roll + ".xtxt", modDeclares.SystemData.LogfileName + "\Roll " + roll + " Index.xlsx")
				IL_43B:
				GoTo IL_4D1
				IL_440:
				...
				IL_4A4:
			Catch obj2 When endfilter(TypeOf obj Is Exception And num4 <> 0 And num6 = 0)
				Dim ex As Exception = CType(obj2, Exception)
				GoTo IL_494
			End Try
			IL_4C6:
		...
		' Token: 0x06000E23 RID: 3619 RVA: 0x00079BA8 File Offset: 0x00077DA8
		<MethodImpl(MethodImplOptions.NoInlining Or MethodImplOptions.NoOptimization)>
		Public Sub ClearLog(ByRef FilmNr As Object)
			Dim num As Integer
			Dim num4 As Integer
			Dim obj As Object
			Try
				...
				IL_07:
				Dim num2 As Integer = 2
				If Not File.Exists(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(modDeclares.SystemData.LogfileName + "\", FilmNr), ".txt"))) Then
					GoTo IL_6B
				End If
				IL_3A:
				num2 = 3
				FileSystem.Kill(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(modDeclares.SystemData.LogfileName + "\", FilmNr), ".txt")))
				IL_6B:
				num2 = 4
				If Not File.Exists(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(modDeclares.SystemData.LogfileName + "\", FilmNr), ".ini"))) Then
					GoTo IL_CF
				End If
				IL_9E:
				num2 = 5
				FileSystem.Kill(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(modDeclares.SystemData.LogfileName + "\", FilmNr), ".ini")))
				IL_CF:
				num2 = 6
				If Not File.Exists(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(MyProject.Application.Info.DirectoryPath + " \ROLLDESCS\", FilmNr), ".desc"))) Then
					GoTo IL_13D
				End If
				IL_107:
				num2 = 7
				FileSystem.Kill(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(MyProject.Application.Info.DirectoryPath + " \ROLLDESCS\", FilmNr), ".desc")))
				IL_13D:
				GoTo IL_1AC
				IL_13F:
				Dim num3 As Integer = num4 + 1
		...
		' Token: 0x06000E24 RID: 3620 RVA: 0x00079D88 File Offset: 0x00077F88
		<MethodImpl(MethodImplOptions.NoInlining Or MethodImplOptions.NoOptimization)>
		Public Sub SetupStartAndEndSymbols()
			Dim num3 As Integer
			Dim num9 As Integer
			Dim obj2 As Object
			Try
				IL_00:
				Dim num As Integer = 1
				If Not modDeclares.SystemData.UseStartSymbole Then
					GoTo IL_2CB
				End If
				IL_11:
				num = 2
				modExpose.AnzStartSymbole = 0
				IL_19:
				num = 3
				Dim text As String = FileSystem.Dir(modDeclares.SystemData.PfadStartSymbole + "\*.*", Microsoft.VisualBasic.FileAttribute.Normal)
				Dim num2 As Integer
				While True
					IL_27B:
					num = 5
					...
					IL_3C:
					num = 6
					Dim text2 As String = modDeclares.SystemData.PfadStartSymbole + "\" + text
					IL_56:
					num = 7
					If Operators.CompareString(Support.Format(Strings.Right(text2, 3), ">", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1), "PDF", False) = 0 Then
						IL_79:
						num = 8
						MyProject.Forms.frmImageXpress.GetPDFPageCount(text2, num2)
					Else
						IL_91:
						num = 10
						num2 = CInt(MyProject.Forms.frmImageXpress.NumPages(text2))
					End If
					IL_A8:
					num = 11
					If num2 > 1 Then
						...
						IL_BA:
						num = 13
						FileSystem.MkDir(modDeclares.SystemData.PfadStartSymbole + "\TEMP")
						IL_D6:
						num = 14
						...
						For i As Integer = 1 To num4
							...
							If Operators.CompareString(Support.Format(Strings.Right(text2, 3), ">", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1), "PDF", False) = 0 Then
								IL_109:
								num = 16
								MyProject.Forms.frmImageXpress.OpenPDFDocumentAlt(text2, i, modDeclares.SystemData.PDFReso, modMain.glImage)
							Else
								IL_131:
								num = 18
								MyProject.Forms.frmImageXpress.OpenRasterDocument(text2, i)
							End If
							IL_148:
							num = 19
							MyProject.Forms.frmImageXpress.SaveRasterFile(String.Concat(New String() { modDeclares.SystemData.PfadStartSymbole, "\TEMP\", text, "_p", Conversions.ToString(i), ".tif" }), 3L)
							IL_19C:
							num = 20
							modExpose.AnzStartSymbole += 1
							IL_1AB:
							num = 21
							modExpose.StartSymbole = CType(Utils.CopyArray(modExpose.StartSymbole, New String(modExpose.AnzStartSymbole + 1 - 1) {}), String())
							IL_1CE:
							num = 22
							modExpose.StartSymbole(modExpose.AnzStartSymbole) = String.Concat(New String() { modDeclares.SystemData.PfadStartSymbole, "\TEMP\", text, "_p", Conversions.ToString(i), ".tif" })
							IL_21B:
							num = 23
						Next
					Else
						...
						IL_261:
						num = 27
						modExpose.StartSymbole(modExpose.AnzStartSymbole) = text2
					End If
					IL_271:
					num = 28
					text = FileSystem.Dir()
				...
				IL_28F:
				num = 30
				If modExpose.AnzStartSymbole <= 0 Then
					GoTo IL_2BD
				End If
				IL_29A:
				num = 31
				Dim sortStyles As modSort.SortStyles = modSort.SortStyles.Style2000
				Dim flag As Boolean = True
				Dim flag2 As Boolean = False
				modSort.SortStrings(modExpose.StartSymbole, modExpose.AnzStartSymbole, sortStyles, flag, flag2)
				GoTo IL_2CB
				IL_2BD:
				num = 33
				modDeclares.SystemData.UseStartSymbole = False
				IL_2CB:
				num = 34
				If Not modDeclares.SystemData.UseEndSymbole Then
					GoTo IL_5B1
				End If
				IL_2DD:
				num = 35
				modExpose.AnzEndSymbole = 0
				IL_2E6:
				num = 36
				text = FileSystem.Dir(modDeclares.SystemData.PfadEndSymbole + "\*.*", Microsoft.VisualBasic.FileAttribute.Normal)
				While True
					IL_560:
					num = 38
					If Operators.CompareString(text, "", False) = 0 Then
					...
					IL_30A:
					num = 39
					Dim text2 As String = modDeclares.SystemData.PfadEndSymbole + "\" + text
					IL_325:
					num = 40
					If Operators.CompareString(Support.Format(Strings.Right(text2, 3), ">", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1), "PDF", False) = 0 Then
						IL_349:
					...
					If num2 > 1 Then
						...
						IL_38B:
						num = 46
						FileSystem.MkDir(modDeclares.SystemData.PfadEndSymbole + "\TEMP")
						IL_3A7:
						num = 47
						Dim num5 As Integer = num2
						For i As Integer = 1 To num5
							IL_3B6:
							num = 48
							If Operators.CompareString(Support.Format(Strings.Right(text2, 3), ">", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1), "PDF", False) = 0 Then
								IL_3DA:
							...
							Else
								IL_402:
								num = 51
								MyProject.Forms.frmImageXpress.LoadRasterFile(text2, i)
							End If
							IL_419:
							num = 52
							MyProject.Forms.frmImageXpress.SaveRasterFile(String.Concat(New String() { modDeclares.SystemData.PfadEndSymbole, "\TEMP\", text, "_p", Conversions.ToString(i), ".tif" }), 3L)
							IL_46D:
							num = 53
							modExpose.AnzEndSymbole += 1
							IL_47C:
							num = 54
							modExpose.EndSymbole = CType(Utils.CopyArray(modExpose.EndSymbole, New String(modExpose.AnzEndSymbole + 1 - 1) {}), String())
							IL_49F:
							num = 55
							modExpose.EndSymbole(modExpose.AnzEndSymbole) = String.Concat(New String() { modDeclares.SystemData.PfadEndSymbole, "\TEMP\", text, "_p", Conversions.ToString(i), ".tif" })
							IL_4EC:
							num = 56
						Next
					Else
						...
						IL_532:
						num = 60
						modExpose.EndSymbole(modExpose.AnzEndSymbole) = modDeclares.SystemData.PfadEndSymbole + "\" + text
					End If
					IL_556:
					num = 61
					text = FileSystem.Dir()
				...
				IL_575:
				num = 63
				If modExpose.AnzEndSymbole <= 0 Then
					GoTo IL_5A3
				End If
				IL_580:
				num = 64
				...
				flag2 = True
				flag = False
				modSort.SortStrings(modExpose.EndSymbole, modExpose.AnzEndSymbole, sortStyles, flag2, flag)
				GoTo IL_5B1
				IL_5A3:
				num = 66
				modDeclares.SystemData.UseEndSymbole = False
				IL_5B1:
				num = 67
				If Not modDeclares.SystemData.UseForsetzungsSymbole1 Then
					GoTo IL_897
				End If
				IL_5C3:
				num = 68
				modExpose.AnzFortsetzungsSymbole1 = 0
				IL_5CC:
				num = 69
				text = FileSystem.Dir(modDeclares.SystemData.PfadForsetzungsSymbole1 + "\*.*", Microsoft.VisualBasic.FileAttribute.Normal)
				While True
					IL_846:
					num = 71
					If Operators.CompareString(text, "", False) = 0 Then
					...
					IL_5F0:
					num = 72
					Dim text2 As String = modDeclares.SystemData.PfadForsetzungsSymbole1 + "\" + text
					IL_60B:
					num = 73
					If Operators.CompareString(Support.Format(Strings.Right(text2, 3), ">", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1), "PDF", False) = 0 Then
						IL_62F:
					...
					If num2 > 1 Then
						...
						IL_671:
						num = 79
						FileSystem.MkDir(modDeclares.SystemData.PfadForsetzungsSymbole1 + "\TEMP")
						IL_68D:
						num = 80
						Dim num6 As Integer = num2
						For i As Integer = 1 To num6
							IL_69C:
							num = 81
							If Operators.CompareString(Support.Format(Strings.Right(text2, 3), ">", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1), "PDF", False) = 0 Then
								IL_6C0:
							...
							IL_6FF:
							num = 85
							MyProject.Forms.frmImageXpress.SaveRasterFile(String.Concat(New String() { modDeclares.SystemData.PfadForsetzungsSymbole1, "\TEMP\", text, "_p", Conversions.ToString(i), ".tif" }), 3L)
							IL_753:
							num = 86
							modExpose.AnzFortsetzungsSymbole1 += 1
							IL_762:
							num = 87
							modExpose.FortsetzungsSymbole1 = CType(Utils.CopyArray(modExpose.FortsetzungsSymbole1, New String(modExpose.AnzFortsetzungsSymbole1 + 1 - 1) {}), String())
							IL_785:
							num = 88
							modExpose.FortsetzungsSymbole1(modExpose.AnzFortsetzungsSymbole1) = String.Concat(New String() { modDeclares.SystemData.PfadForsetzungsSymbole1, "\TEMP\", text, "_p", Conversions.ToString(i), ".tif" })
							IL_7D2:
							num = 89
						Next
					Else
						...
						IL_818:
						num = 93
						modExpose.FortsetzungsSymbole1(modExpose.AnzFortsetzungsSymbole1) = modDeclares.SystemData.PfadForsetzungsSymbole1 + "\" + text
					End If
					IL_83C:
					num = 94
					text = FileSystem.Dir()
				...
				IL_85B:
				num = 96
				If modExpose.AnzFortsetzungsSymbole1 <= 0 Then
					GoTo IL_889
				End If
				IL_866:
				num = 97
				...
				flag = True
				flag2 = False
				modSort.SortStrings(modExpose.FortsetzungsSymbole1, modExpose.AnzFortsetzungsSymbole1, sortStyles, flag, flag2)
				GoTo IL_897
				IL_889:
				num = 99
				modDeclares.SystemData.UseForsetzungsSymbole1 = False
				IL_897:
				num = 100
				If Not modDeclares.SystemData.UseForsetzungsSymbole2 Then
					GoTo IL_B86
				End If
				IL_8A9:
				num = 101
				modExpose.AnzFortsetzungsSymbole2 = 0
				IL_8B2:
				num = 102
				text = FileSystem.Dir(modDeclares.SystemData.PfadForsetzungsSymbole2 + "\*.*", Microsoft.VisualBasic.FileAttribute.Normal)
				While True
					IL_B2C:
					num = 104
					If Operators.CompareString(text, "", False) = 0 Then
					...
					IL_8D6:
					num = 105
					Dim text2 As String = modDeclares.SystemData.PfadForsetzungsSymbole2 + "\" + text
					IL_8F1:
					num = 106
					If Operators.CompareString(Support.Format(Strings.Right(text2, 3), ">", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1), "PDF", False) = 0 Then
						IL_915:
					...
					If num2 > 1 Then
						...
						IL_957:
						num = 112
						FileSystem.MkDir(modDeclares.SystemData.PfadForsetzungsSymbole2 + "\TEMP")
						IL_973:
						num = 113
						Dim num7 As Integer = num2
						For i As Integer = 1 To num7
							IL_982:
							num = 114
							If Operators.CompareString(Support.Format(Strings.Right(text2, 3), ">", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1), "PDF", False) = 0 Then
								IL_9A6:
							...
							IL_9E5:
							num = 118
							MyProject.Forms.frmImageXpress.SaveRasterFile(String.Concat(New String() { modDeclares.SystemData.PfadForsetzungsSymbole2, "\TEMP\", text, "_p", Conversions.ToString(i), ".tif" }), 3L)
							IL_A39:
							num = 119
							modExpose.AnzFortsetzungsSymbole2 += 1
							IL_A48:
							num = 120
							modExpose.FortsetzungsSymbole2 = CType(Utils.CopyArray(modExpose.FortsetzungsSymbole2, New String(modExpose.AnzFortsetzungsSymbole2 + 1 - 1) {}), String())
							IL_A6B:
							num = 121
							modExpose.FortsetzungsSymbole2(modExpose.AnzFortsetzungsSymbole2) = String.Concat(New String() { modDeclares.SystemData.PfadForsetzungsSymbole2, "\TEMP\", text, "_p", Conversions.ToString(i), ".tif" })
							IL_AB8:
							num = 122
						Next
					Else
						...
						IL_AFE:
						num = 126
						modExpose.FortsetzungsSymbole2(modExpose.AnzFortsetzungsSymbole2) = modDeclares.SystemData.PfadForsetzungsSymbole2 + "\" + text
					End If
					IL_B22:
					num = 127
					text = FileSystem.Dir()
				...
				flag2 = True
				flag = False
				modSort.SortStrings(modExpose.FortsetzungsSymbole2, modExpose.AnzFortsetzungsSymbole2, sortStyles, flag2, flag)
				GoTo IL_B86
				IL_B75:
				num = 132
				modDeclares.SystemData.UseForsetzungsSymbole2 = False
				IL_B86:
				num = 133
				modExpose.AnzGesamtEndSymbole = modExpose.AnzEndSymbole
				IL_B96:
				num = 134
				modExpose.GesamtEndSymbole = CType(Utils.CopyArray(modExpose.GesamtEndSymbole, New String(modExpose.AnzGesamtEndSymbole + 1 - 1) {}), String())
				IL_BBC:
				num = 135
				Dim obj As Object
				Dim loopObj As Object
				If Not ObjectFlowControl.ForLoopControl.ForLoopInitObj(obj, 1, modExpose.AnzEndSymbole, 1, loopObj, obj) Then
					GoTo IL_C14
				End If
				Do
					IL_BE4:
					num = 136
					modExpose.GesamtEndSymbole(Conversions.ToInteger(obj)) = modExpose.EndSymbole(Conversions.ToInteger(obj))
					IL_C02:
					num = 137
				Loop While ObjectFlowControl.ForLoopControl.ForNextCheckObj(obj, loopObj, obj)
				IL_C14:
				num = 138
				modExpose.AnzGesamtStartSymbole = modExpose.AnzStartSymbole
				IL_C24:
				num = 139
				modExpose.GesamtStartSymbole = CType(Utils.CopyArray(modExpose.GesamtStartSymbole, New String(modExpose.AnzGesamtStartSymbole + 1 - 1) {}), String())
				IL_C4A:
				num = 140
				Dim loopObj2 As Object
				If Not ObjectFlowControl.ForLoopControl.ForLoopInitObj(obj, 1, modExpose.AnzGesamtStartSymbole, 1, loopObj2, obj) Then
					GoTo IL_CA2
				End If
				Do
					IL_C72:
					num = 141
					modExpose.GesamtStartSymbole(Conversions.ToInteger(obj)) = modExpose.StartSymbole(Conversions.ToInteger(obj))
					IL_C90:
					num = 142
				Loop While ObjectFlowControl.ForLoopControl.ForNextCheckObj(obj, loopObj2, obj)
				IL_CA2:
				num = 143
				modExpose.AnzSymboleAufFilm = modExpose.AnzStartSymbole + modExpose.AnzEndSymbole
				IL_CB8:
				GoTo IL_F4A
				IL_CBD:
				...
				switch(ICSharpCode.Decompiler.ILAst.ILLabel[], num3)
				IL_F1D:
			Catch obj3 When endfilter(TypeOf obj2 Is Exception And num3 <> 0 And num9 = 0)
		...
		' Token: 0x06000E25 RID: 3621 RVA: 0x0007AD04 File Offset: 0x00078F04
		Private Function WaitForProcessor(ByRef NewFileName As String, pag As Integer) As String
			Dim text As String = MyProject.Application.Info.DirectoryPath + "\docufile.ini"
			Dim text2 As String = Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(modMain.GiveIni(text, "SYSTEM", "RAMDRIVE"), "\"), Strings.Mid(NewFileName, Strings.InStrRev(NewFileName, "\", -1, Microsoft.VisualBasic.CompareMethod.Binary) + 1)))
			text2 = Strings.Left(text2, Strings.Len(text2) - 4) + "_" + pag.ToString()
			text2 + ".ok"
			text2 += ".tif"
		...
		' Token: 0x06000E26 RID: 3622 RVA: 0x0007ADD0 File Offset: 0x00078FD0
		Private Function WaitForProcessor2(ByRef NewFileName As String) As String
			Dim text As String = MyProject.Application.Info.DirectoryPath + "\docufile.ini"
			Dim text2 As String = Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(modMain.GiveIni(text, "SYSTEM", "RAMDRIVE"), "\"), Strings.Mid(NewFileName, Strings.InStrRev(NewFileName, "\", -1, Microsoft.VisualBasic.CompareMethod.Binary) + 1)))
			text2 = Strings.Left(text2, Strings.Len(text2) - 3) + "tif"
			Dim result As String = text2
			Dim fileSystemObject As FileSystemObject = New FileSystemObjectClass()
			Dim num As Integer = 50
			While True
				Application.DoEvents()
				If fileSystemObject.FileExists(text2) Then
					Exit For
				End If
				modDeclares.Sleep(100)
				num -= 1
			...
			While True
				Application.DoEvents()
				If Not modExpose.IsFileInUse(text2) > modDeclares.IsFileResults.FILE_FREE Then
					Exit For
				End If
				modDeclares.Sleep(100)
				num -= 1
		...
		' Token: 0x06000E27 RID: 3623 RVA: 0x0007AEB4 File Offset: 0x000790B4
		Private Function IsFileInUse(ByRef sFile As String) As modDeclares.IsFileResults
			Dim security_ATTRIBUTES As modDeclares.SECURITY_ATTRIBUTES
			security_ATTRIBUTES.nLength = 0
			Dim result As modDeclares.IsFileResults
			If modExpose.FileExists(sFile) Then
				Dim num As Integer = modDeclares.CreateFile(sFile, Integer.MinValue, 0, security_ATTRIBUTES, 3, 128, 0)
				Dim right As Object
				result = CType(Conversions.ToInteger(Operators.CompareObjectEqual(num, right, False)), modDeclares.IsFileResults)
				modDeclares.CloseHandle(num)
			Else
				result = modDeclares.IsFileResults.FILE_DOESNT_EXIST
			End If
			Return result
		End Function

		' Token: 0x06000E28 RID: 3624 RVA: 0x0007AF08 File Offset: 0x00079108
		Private Function IsFileInUse2(ByRef sFile As String) As modDeclares.IsFileResults
			Dim security_ATTRIBUTES As modDeclares.SECURITY_ATTRIBUTES
			security_ATTRIBUTES.nLength = 0
			Dim result As modDeclares.IsFileResults
			If modExpose.FileExists(sFile) Then
		...
		' Token: 0x06000E29 RID: 3625 RVA: 0x0007AF5C File Offset: 0x0007915C
		Public Function FileExists(ByRef sSource As String) As Boolean
			Return File.Exists(sSource)
		End Function

		' Token: 0x06000E2A RID: 3626 RVA: 0x0007AF74 File Offset: 0x00079174
		<MethodImpl(MethodImplOptions.NoInlining Or MethodImplOptions.NoOptimization)>
		Private Sub AddVacError(ByRef Rolle As String, ByRef corimg As Integer, ByRef Level As Short, ByRef DokumentName As String, ByRef page As Integer, posinmm As Double)
			New FileSystemObjectClass()
			Dim flag As Boolean = False
			If Not Directory.Exists(Application.StartupPath + "\EXTLOGS") Then
				Directory.CreateDirectory(Application.StartupPath + "\EXTLOGS")
			End If
			If Not File.Exists(Application.StartupPath + "\EXTLOGS\" + Rolle + ".txt") Then
				flag = True
			End If
			Dim num As Short = CShort(FileSystem.FreeFile())
			FileSystem.FileOpen(CInt(num), MyProject.Application.Info.DirectoryPath + "\EXTLOGS\" + Rolle + ".txt", OpenMode.Append, OpenAccess.[Default], OpenShare.[Default], -1)
			If flag Then
				FileSystem.PrintLine(CInt(num), New Object() { "Vacuum Particularities" })
				FileSystem.PrintLine(CInt(num), New Object() { "=======================" })
			End If
			FileSystem.PrintLine(CInt(num), New Object() { "Location on Film: " + Support.Format(posinmm, "##0.###", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1) + " m" })
			If Level = 2S Then
				FileSystem.PrintLine(CInt(num), New Object() { "Correction Image# " + Support.Format(corimg, "", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1) + " exposed" })
			End If
			If Level = 3S Then
				FileSystem.PrintLine(CInt(num), New Object() { "Automatic Correction failed" })
			End If
			FileSystem.PrintLine(CInt(num), New Object() { String.Concat(New String() { "Source Image: ", DokumentName, "(", Support.Format(page, "", FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1), "); " }) })
			FileSystem.PrintLine(CInt(num), New Object() { "=======================" })
			FileSystem.FileClose(New Integer() { CInt(num) })
		End Sub

		' Token: 0x06000E2B RID: 3627 RVA: 0x0007B120 File Offset: 0x00079320
		Public Function StartFirstRenderer(ind As Integer) As Integer
			Dim images As modDeclares.typImage() = modDeclares.Images
			Dim result As Integer = -1
			Dim imagecount As Integer = modDeclares.imagecount
			For i As Integer = ind To imagecount
				Dim images2 As modDeclares.typImage() = modDeclares.Images
				If modDeclares.Images(i).IsPDF And modDeclares.Images(i).page = 1 And Not modDeclares.Images(i).RendererStarted Then
					modDeclares.Images(i).RendererStarted = True
					result = i
				End If
			Next
			Return result
		...
		' Token: 0x06000E2C RID: 3628 RVA: 0x0007B1A8 File Offset: 0x000793A8
		Public Function StartNextRenderer(ind As Integer) As Integer
			Dim images As modDeclares.typImage() = modDeclares.Images
			Dim result As Integer = -1
			Dim imagecount As Integer = modDeclares.imagecount
			For i As Integer = ind To imagecount
				Dim name As String = modDeclares.Images(i).Name
				If modDeclares.Images(i).IsPDF And modDeclares.Images(i).page = 1 And Not modDeclares.Images(i).RendererStarted Then
					Dim text As String = MyProject.Application.Info.DirectoryPath + "\'PDFRenderer.exe "
					Dim text2 As String = """" + name + """ 1 0 " + Conversions.ToString(modDeclares.SystemData.PDFReso)
					modMonitorTest.ExecCmdNoWait2(text, text2)
					modDeclares.Images(i).RendererStarted = True
					result = i
				End If
			Next
		...
		Private StartSymbole As String()

		' Token: 0x040008BA RID: 2234
		Private AnzStartSymbole As Integer

		' Token: 0x040008BB RID: 2235
		Private EndSymbole As String()

		' Token: 0x040008BC RID: 2236
		Private AnzEndSymbole As Integer

		' Token: 0x040008BD RID: 2237
		Private GesamtStartSymbole As String()

		' Token: 0x040008BE RID: 2238
		Private AnzGesamtStartSymbole As Integer

		' Token: 0x040008BF RID: 2239
		Private GesamtEndSymbole As String()

		' Token: 0x040008C0 RID: 2240
		Private AnzGesamtEndSymbole As Integer

		' Token: 0x040008C1 RID: 2241
		Private FortsetzungsSymbole1 As String()

		' Token: 0x040008C2 RID: 2242
		Private FortsetzungsSymbole2 As String()

		' Token: 0x040008C3 RID: 2243
		Private AnzFortsetzungsSymbole1 As Integer

		' Token: 0x040008C4 RID: 2244
		Private AnzFortsetzungsSymbole2 As Integer

		' Token: 0x040008C5 RID: 2245
		Private ExposeEndSymbols As Boolean

		' Token: 0x040008C6 RID: 2246
		Private AnzSymboleAufFilm As Integer

		' Token: 0x040008C7 RID: 2247
		Private clonimage As Image
	End Module
End Namespace
